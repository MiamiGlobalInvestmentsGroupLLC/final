...
export default function ClientLoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        window.location.href = "/dashboard"; // غيّرها لمسار داشبورد الشركات عندك
      } else {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-2xl shadow p-6 bg-white">
        <h1 className="text-2xl font-bold mb-4 text-center">Company Login</h1>
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            <label className="block text-sm mb-1">Email</label>
            <input type="email" required value={email} onChange={e=>setEmail(e.target.value)}
              className="w-full border rounded px-3 py-2 outline-none" placeholder="you@company.com"/>
          </div>
          <div>
